...
class RoomReservationDataAccess(BaseDataAccess):
    ...
    def load(self):
        ...
        for id in self.m_config_parser.sections():
            ...
            r.set_user_id(self.m_config_parser[id]["user id"])
            r.set_reserved_room_id(self.m_config_parser[id]["room id"])
    ...
    def add_reservation(self, rsv):
        ...
        newrsv.set_user_id(rsv.get_user_id())
        ...
        self.m_config_parser[newid]["user id"] = rsv.get_user_id()
        self.m_config_parser[newid]["room id"] = rsv.get_reserved_room_id()
        ...
        if (self.m_config_parser.has_section(rsv.get_id())):
            ...
            for existing_rsv in self.m_reservations:
                if existing_rsv.get_id()== rsv.get_id():
                    ...
                    existing_rsv.set_user_id(rsv.get_user_id())
            ...
            self.m_config_parser[rsv.get_id()]["user id"] = rsv.get_user_id()
            self.m_config_parser[rsv.get_id()]["room id"] = rsv.get_reserved_room_id()
            return True
        else :
            return False
